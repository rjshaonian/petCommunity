...
import com.tedu.petCommunity.common.rocketmq.init.RocketmqProducerInit;

/**
 * 消息生产者，可与消费者分离； 消息生产的三种方式：可靠同步发送，可靠异步发送，单向发送
 * 
 * @author 阳昊 2020年3月3日 下午6:13:25
 */
@Service
public class RocketmqProducerService {
	...
	/**
	 * 同步发送实体对象消息 可靠同步发送：同步发送是指消息发送方发出数据后，会在收到接收方发回响应之后才发下一个数据包的通讯方式；
	...
	public boolean sendMsg(String msg) {
		...
		if (sendResult != null) {
			System.out.println(new Date() + " Send mq message success. Topic is:" + message.getTopic() + " msgId is: "
	...
	/**
	 * 异步发送消息 可靠异步发送：发送方发出数据后，不等接收方发回响应，接着发送下个数据包的通讯方式； 特点：速度快；有结果反馈；数据可靠；
	...
	/**
	 * 单向发送 单向发送：只负责发送消息，不等待服务器回应且没有回调函数触发，即只发送请求不等待应答；此方式发送消息的过程耗时非常短，一般在微秒级别；
	 * 特点：速度最快，耗时非常短，毫秒级别；无结果反馈；数据不可靠，可能会丢失； 应用场景：适用于某些耗时非常短，但对可靠性要求并不高的场景，例如日志收集；
	 * 
	 * @return
	 */
